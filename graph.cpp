...
public:
  ...
  void removeEdgeList(int src, int dest) {
    ...
    if (!directed) {
      for (auto it = adjacencyList[dest].begin();
           it != adjacencyList[dest].end(); ++it) {
        if (*it == src) {
  ...
  // Função para adicionar várias arestas com peso usando uma lista de pares de
  // vértices e peso
  ...
  vector<int> getNeighborsList(int vertex) { return adjacencyList[vertex]; }

  // Função para verificar os sucessores de um vértice na matriz de adjacência
  ...
  // Função para verificar os sucessores de um vértice na lista de adjacência
  // (para grafo direcionado)
  vector<int> getSuccessorsList(int vertex) { return adjacencyList[vertex]; }

  // Função para verificar os predecessores de um vértice na matriz de
  // adjacência (para grafo direcionado)
  ...
  // Função para verificar os predecessores de um vértice na lista de adjacência
  // (para grafo direcionado)
  ...
  int getDegreeList(int vertex) { return adjacencyList[vertex].size(); }
  ...
  bool isSimpleList() {
    for (int i = 1; i <= numVertices; ++i) {
      for (int j : adjacencyList[i]) {
        if (j == i ||
            count(adjacencyList[j].begin(), adjacencyList[j].end(), i) > 0) {
          return false; // Se houver laços ou arestas paralelas, o grafo não é
                        // simples
        }
      }
    }
  ...
  bool isRegularList() {
    ...
    for (int i = 2; i <= numVertices; ++i) {
      if (adjacencyList[i].size() != degree) {
        return false; // Se o grau de qualquer vértice for diferente do grau do
                      // primeiro vértice, o grafo não é regular
      }
    }
    return true; // Grafo é regular
  }
  ...
  bool isCompleteList() {
    for (int i = 1; i <= numVertices; ++i) {
      if (adjacencyList[i].size() != numVertices - 1) {
        return false; // Se o tamanho da lista de adjacência de qualquer vértice
                      // não for (numVertices - 1), o grafo não é completo
      }
    }
    return true; // Grafo é completo
  }
  ...
  bool isBipartiteList() {
    vector<int> color(numVertices + 1,
                      -1); // Inicializa todos os vértices sem cor (-1)
    for (int i = 1; i <= numVertices; ++i) {
        ...
        while (!q.empty()) {
          ...
          for (int v : adjacencyList[u]) {
            if (color[v] == -1) { // Se o vértice não foi visitado
              color[v] =
                  1 - color[u]; // Atribui a cor oposta ao vértice adjacente
              q.push(v);
            } else if (color[v] ==
                       color[u]) { // Se o vértice adjacente tiver a mesma cor
        ...
        while (!q.empty()) {
          ...
          for (int v = 1; v <= numVertices; ++v) {
            if (adjacencyMatrix[u][v] ==
                1) {                // Se houver uma aresta de u para v
              if (color[v] == -1) { // Se o vértice não foi visitado
                color[v] =
                    1 - color[u]; // Atribui a cor oposta ao vértice adjacente
                q.push(v);
  ...
  void breadthFirstSearchList(int startVertex) {
    ...
    // Array para armazenar o nível de cada vértice em relação ao vértice
    // inicial
    ...
    while (!q.empty()) {
      ...
      for (int neighbor : neighbors) {
        ...
        if (!visited[neighbor]) {
          ...
          // Define o nível do vizinho como sendo 1 nível mais do que o nível do
          // vértice atual
          level[neighbor] = level[currentVertex] + 1;
          // Define o pai do vizinho como o vértice atual
          parent[neighbor] = currentVertex;
          // Adiciona o vizinho à fila para explorar seus vizinhos
          // posteriormente
          q.push(neighbor);
        }
      }
    ...
    // Mostra os níveis, pais e ordem de visita dos vértices após a busca em
    // largura
    cout << "Níveis, pais e ordem de visita dos vértices a partir do vértice "
         << startVertex << ":\n";
    for (int i = 1; i <= numVertices; ++i) {
      cout << "Vértice " << i << ": Nível " << level[i] << ", Pai " << parent[i]
           << ", Visitado em ordem " << visitOrder[i - 1] << endl;
    }
  }
      ...
      for (int i = 1; i <= numVertices; ++i) {
        ...
        if (adjacencyMatrix[currentVertex][i] == 1) {
          ...
          if (!visited[i]) {
            ...
            visited[i] = true;
            // Define o nível do vértice i como sendo 1 nível mais do que o
            // nível do vértice atual
            level[i] = level[currentVertex] + 1;
            // Define o pai do vértice i como o vértice atual
            parent[i] = currentVertex;
            // Adiciona o vértice i à fila para explorar seus vizinhos
            // posteriormente
            q.push(i);
          }
        }
  ...
  void depthFirstSearchMatrix(int vertex, bool visited[], int discoveryTime[],
                              int finishTime[], int parent[]) {
    ...
    // Para cada vértice adjacente ao vértice atual
    for (int i = 1; i <= numVertices; ++i) {
      // Se houver uma aresta do vértice atual para o vértice i e o vértice i
  ...
  void depthFirstSearchList(int vertex, bool visited[], int discoveryTime[],
                            int finishTime[], int parent[]) {
    // Marca o vértice como visitado
    visited[vertex] = true;
    ...
    for (int neighbor : neighbors) {
      ...
      if (!visited[neighbor]) {
        ...
        depthFirstSearchList(neighbor, visited, discoveryTime, finishTime,
                             parent);
      }
    }
    ...
    while (!fila.empty()) {
      ...
      for (int i = 1; i <= numVertices; ++i) {
        if (adjacencyMatrix[atual][i] == 1) {
          ...
          grauEntrada[i]--;
          // Se o grau de entrada do vértice adjacente se tornar zero, o
          // adiciona à fila
          if (grauEntrada[i] == 0) {
            fila.push(i);
          }
  ...
  vector<int> AGMPrimMatrix() {
    ...
    // Define o custo mínimo do primeiro vértice como 0 para iniciar a AGM a
    // partir dele
    custoMinimo[1] = 0;

    // Loop para adicionar vértices à AGM
    for (int i = 1; i <= numVertices; ++i) {
      // Encontra o vértice com o custo mínimo que ainda não está incluído na
      // AGM
      int verticeMinimo = -1;
      for (int j = 1; j <= numVertices; ++j) {
        if (!incluido[j] && (verticeMinimo == -1 ||
                             custoMinimo[j] < custoMinimo[verticeMinimo])) {
          verticeMinimo = j;
        }
      }
      ...
      // Atualiza os custos mínimos dos vértices adjacentes ao vértice
      // adicionado à AGM
      for (int j = 1; j <= numVertices; ++j) {
        if (adjacencyMatrix[verticeMinimo][j] != 0 && !incluido[j] &&
            adjacencyMatrix[verticeMinimo][j] < custoMinimo[j]) {
  ...
  int encontrarConjunto(vector<int> &conjuntos, int elemento) {
    // Se o elemento não pertence ao conjunto dele mesmo, recursivamente
    // encontra o conjunto de seu pai
  ...
  int encontrarMinimaDistancia(vector<int> &distancia, vector<bool> &processado,
                               int numVertices) {
  ...
  vector<int> caminhoMinimoMatrix(int origem, int destino) {
    vector<int> distancia(
        numVertices + 1, INT_MAX); // Vetor para armazenar as distâncias mínimas
    vector<bool> processado(
        numVertices + 1, false); // Vetor para marcar os vértices já processados
    vector<int> pai(
        numVertices + 1,
        -1); // Vetor para armazenar os pais de cada vértice no caminho mínimo
    ...
    for (int count = 1; count <= numVertices - 1; count++) {
      int u = encontrarMinimaDistancia(
          distancia, processado,
          numVertices); // Encontrar o vértice com a menor distância ainda não
                        // processado
      processado[u] = true; // Marcar o vértice como processado

      // Atualizar as distâncias dos vértices adjacentes ao vértice escolhido
      for (int v = 1; v <= numVertices; v++) {
        if (!processado[v] && adjacencyMatrix[u][v] &&
            distancia[u] != INT_MAX &&
            distancia[u] + adjacencyMatrix[u][v] < distancia[v]) {
    ...
    reverse(caminho.begin(),
            caminho.end()); // Inverter o caminho para obter a ordem correta
  ...
  vector<int> caminhoMinimoList(int origem, int destino) {
    ...
    vector<int> pai(
        numVertices + 1,
        -1); // Vetor para armazenar os pais de cada vértice no caminho mínimo
    vector<bool> visitado(numVertices + 1,
                          false); // Vetor para marcar os vértices já visitados
    ...
    while (!fila.empty()) {
      ...
      for (int i = 0; i < adjacencyList[u].size(); ++i) {
        ...
        // Se a distância até o vizinho passando pelo vértice atual é menor do
        // que a distância atual armazenada para o vizinho
        if (!visitado[v] && distancia[u] + pesoAtual < distancia[v]) {
          distancia[v] =
              distancia[u] + pesoAtual; // Atualiza a distância mínima
    ...
    reverse(caminho.begin(),
            caminho.end()); // Inverte o caminho para obter a ordem correta
